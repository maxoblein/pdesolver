...
def test_forward_unstable():
    mx = 40
    mt = 1000
    u_T = Finite_Difference('forward',initial_cond,[0,0],40,1000,(1.0,1.0,0.5),b_type = [0,0])
    if u_T == [1,1]:
...
def test_forward_output():
    tol = 1e-3
    u_T,diagnostics = Finite_Difference('forward',initial_cond,[0,0],10,1000,(1.0,1.0,0.5),b_type = [0,0])
...
def test_backward_output():
    tol = 1e-4
    u_T,diagnostics = Finite_Difference('backward',initial_cond,[0,0],40,1000,(1.0,1.0,0.5),b_type = [0,0])
...
def test_crank_output():
    tol = 1e-4
    u_T,diagnostics = Finite_Difference('crank',initial_cond,[0,0],40,1000,(1.0,1.0,0.5),b_type = [0,0])
    x = np.linspace(0,1,41)
    u_e = u_exact(x,0.5,(1.0,1.0,0.5))
    error = abs(np.sum(u_e - u_T)/41)
    if error < tol:
...
def test_incorrect_b_type():
    u_T_forward = Finite_Difference('forward',initial_cond,[0,0],10,1000,(1.0,1.0,0.5),b_type = [2,2])
    u_T_backward = Finite_Difference('backward',initial_cond,[0,0],40,1000,(1.0,1.0,0.5),b_type = [2,2])
    u_T_crank = Finite_Difference('crank',initial_cond,[0,0],40,1000,(1.0,1.0,0.5),b_type = [2,2])
